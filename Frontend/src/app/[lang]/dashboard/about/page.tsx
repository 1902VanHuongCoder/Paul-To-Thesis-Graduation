...
export default function AboutPage() {
  ...
  useEffect(() => {
    fetch(`${baseUrl}/api/about`)
      .then(res => res.json())
      .then(data => {
         console.log("Fetched about records:", data);
         setAboutList([data]);
      });
  }, []);
  ...
  const handleImageChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (oldCompanyImage && oldCompanyImage !== imagePreview) {
      await fetch(`${baseUrl}/api/upload/single-delete`, {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ url: oldCompanyImage }),
      });
    }
    const url = await uploadAvatar(file);
    ...
    if (editAboutIndex !== null) {
      ...
      if (oldAbout.companyLogo && oldAbout.companyLogo !== data.companyLogo) {
        await fetch(`${baseUrl}/api/upload/single-delete`, {
          method: "DELETE",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ url: oldAbout.companyLogo }),
        });
      }
      ...
      if (oldAbout.companyImage && oldAbout.companyImage !== data.companyImage) {
        await fetch(`${baseUrl}/api/upload/single-delete`, {
          method: "DELETE",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ url: oldAbout.companyImage }),
        });
      }
      // Update
        ...
        <tbody>
          {aboutList.map((about, idx) => (
            <tr key={idx}>
              ...
              <td>{about.companyLogo && <img src={about.companyLogo} alt="Logo" className="h-8" />}</td>
              <td>
          ...
          <FormProvider {...methods}>
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              ...
              <FormItem>
                ...
                {logoPreview && <img src={logoPreview} alt="Logo" className="w-20 h-20 object-cover mt-2" />}
              </FormItem>
              <FormItem>
                ...
                {imagePreview && <img src={imagePreview} alt="Company" className="w-20 h-20 object-cover mt-2" />}
              </FormItem>
              <FormItem>

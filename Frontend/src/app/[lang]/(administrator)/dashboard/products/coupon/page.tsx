...
import { createDiscount, deleteDiscount, fetchAllDiscounts, updateDiscount } from "@/lib/discount-apis";
...
export default function DiscountPage() {
  ...
  const onSubmit = async (data: Discount) => {
    ...
    try {
      if (editDiscount) {
        const res = await updateDiscount(payload);
        if (!res) {
        ...
        const resData = await createDiscount(payload);
  ...
  const handleToggleStatus = async (discount: Discount) => {
    try {
      ...
      const res = await updateDiscount(updated);

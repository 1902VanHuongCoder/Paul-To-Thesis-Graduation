...
import { createTagOfNews, deleteTagOfNews, fetchTagsOfNews, updateTagOfNews } from "@/lib/tag-of-news-apis";
...
const AddTagOfNews = () => {
    ...
    const onSubmit = async (data: { tagName: string }) => {
        setLoading(true);
        try {
            const res = await createTagOfNews(data.tagName);
            if (!res) {
    ...
    const confirmDelete = async () => {
        if (!deleteID) return;
        try {
            const res = await deleteTagOfNews(deleteID);
            if (res) {
    ...
    const handleEditSave = async (id: number) => {
        try {
            const res = await updateTagOfNews(id, editName);
            if (res) {
